...
/* ---------- 홈: 자료실 미리보기(3종) ---------- */
function renderResourcesTeaser() {
  const mount = document.getElementById('resources-teaser-mount');
  if (!mount) return;
  const r = SITE.resources;
  const preview = r.items.slice(0, 3);
  mount.innerHTML = `
    ...
    <div class="res-grid">
      ${preview.map(it => `
        <a class="res-card" href="${it.href}" ${it.type === 'external' ? 'target="_blank" rel="noopener"' : 'download'}>
          <span class="res-card__icon">${resIcon(it.icon)}</span>
          <span class="res-card__badge">${it.badge}</span>
          <h4>${it.title}</h4>
          <p>${it.desc}</p>
        </a>
      `).join('')}
    ...
    <div class="fact-grid-more">
      <a href="resources.html">자료실 전체 보기 →</a>
    </div>
  `;
}
...
function renderFloatingCtaAndModal() {
  ...
  fab.innerHTML = `
    ...
    <button type="button" class="fab-main" id="fab-open-btn" aria-label="${f.label}">
  ...
  document.getElementById('fab-open-btn').addEventListener('click', openModal);
...
async function handleQuickFormSubmit(e) {
  ...
  if (q.formspreeId === 'YOUR_FORM_ID') {
    console.warn('[자림인포시스템] Formspree ID가 아직 설정되지 않았습니다. js/site-data.js의 quickForm.formspreeId를 실제 발급받은 ID로 교체해주세요.');
  }
...
function renderMobileStickyBar() {
  ...
  bar.innerHTML = `
    ...
    <button type="button" class="msb-btn msb-btn--solid" data-open-quickform>상담 문의</button>
  `;
